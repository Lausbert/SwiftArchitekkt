...
extension XcodeBuildWrapper {
    ...
    static func update(graphRequest: GraphRequest, completionHandler: (GraphRequest.Result) -> Void) -> GraphRequest? {
        while let mostUrgentMissingParamter = XcodeBuildWrapper.mostUrgentMissingParameter(for: graphRequest) {
            do {
                let parameterOptions = try XcodeBuildWrapper.get(parameter: mostUrgentMissingParamter, for: graphRequest)
                if let options = parameterOptions[mostUrgentMissingParamter.rawValue], options.count == 1, let singleOption = options.first {
                    return GraphRequest(url: graphRequest.url, options: graphRequest.options.merging([mostUrgentMissingParamter.rawValue: singleOption], uniquingKeysWith: { $1 } ))
                } else {
                    completionHandler(GraphRequest.Result.decisionNeeded(graphRequest, parameterOptions))
                    return nil
                }
            } catch {
