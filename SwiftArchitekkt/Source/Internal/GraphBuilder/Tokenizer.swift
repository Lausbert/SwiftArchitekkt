...
class Tokenizer {
    ...
    enum ErrorEnum: LocalizedError, Equatable {
        ...
        var errorDescription: String? {
            switch self {
            case .missingIdentifier(let rawToken):
    ...
    enum Token: CustomStringConvertible, Equatable {
        ...
        var description: String {
            ...
            case .importDeclaration(let identifier):
    ...
    private var identifierPrefix: String {
        for openScope in openScopes.reversed() {
            if case let .scopeStart(_, identifier) = openScope, let id = identifier {
                return id + "."
            }
        }
        return ""
    ...
    private func scopeStartToken(with rawToken: RawTokenizer.RawToken) throws -> Token {
        unclosedLeftParenthesisCount -= 1
        var scopeStart: Token
        switch rawToken {
        case .classDeclaration,
             .funcDeclaration,
             .varDeclaration,
             .parameter:
            if let nextRawToken = nextRawToken(), case var .identifier(identifier) = nextRawToken {
                identifier = identifierPrefix + identifier
                scopeStart = Token.scopeStart(rawToken, identifier: identifier)
            } else {
                throw ErrorEnum.missingIdentifier(rawToken)
            }
        default:
            scopeStart = Token.scopeStart(rawToken, identifier: nil)
        }
    ...
    private func tokenWithIdentifier(for initialRawToken: RawTokenizer.RawToken) throws -> Token {
        while let rawToken = nextRawToken() {
            ...
            case let .identifier(identifier):
                ...
                case .importDeclaration:
                    return .importDeclaration(identifier)
                case .type:
                    return .type(identifier)
                case .apiName:
                    return .apiName(identifier)
